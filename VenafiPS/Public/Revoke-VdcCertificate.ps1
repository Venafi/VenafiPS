function Revoke-VdcCertificate {
    <#
    .SYNOPSIS
    ...
    .EXAMPLE
    $cert | Revoke-VdcCertificate -Reason 2
    Revoke the certificate with a reason of the CA being compromised

    .EXAMPLE
    $cert | Revoke-VdcCertificate -Force
    Revoke the certificate bypassing the confirmation prompt

    .EXAMPLE
    Revoke-VdcCertificate -Path '\VED\Policy\My folder\app.mycompany.com' -Reason 2 -Wait
    Revoke the certificate with a reason of the CA being compromised and wait for it to complete

    .LINK
    http://VenafiPS.readthedocs.io/en/latest/functions/Revoke-VdcCertificate/

    .LINK
    https://github.com/Venafi/VenafiPS/blob/main/VenafiPS/Public/Revoke-VdcCertificate.ps1
