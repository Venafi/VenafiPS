function Get-VenafiCertificate {
    ...
    param (
        ...
        [Parameter(ParameterSetName = 'VaasId', Mandatory, ValueFromPipelineByPropertyName, Position = 0)]
        [Parameter(ParameterSetName = 'TppId', Mandatory, ValueFromPipelineByPropertyName, Position = 0)]
        [Alias('Guid', 'Path', 'id')]
    ...
    process {
        ...
        switch ($platform) {
            'VaaS' {
                ...
                if ( $All ) {
                    return (Find-VcCertificate -IncludeVaasOwner:$IncludeVaasOwner -VenafiSession $VenafiSession)
                }
            ...
            Default {
                ...
                if ( $All ) {
                    return (Find-VdcCertificate -Path '\ved' -Recursive -VenafiSession $VenafiSession |
                ...
                if ( [guid]::TryParse($CertificateId, $([ref][guid]::Empty)) ) {
                    ...
                    # a path was provided
                    $thisGuid = $CertificateId | ConvertTo-VdcFullPath | ConvertTo-VdcGuid -VenafiSession $VenafiSession
                }
